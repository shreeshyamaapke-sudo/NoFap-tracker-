...
export default function NoFapTracker() {
  // ✅ localStorage se load hoga — refresh ke baad bhi data rahega
  const [relapses, setRelapses] = useState(() => {
    try {
      const saved = localStorage.getItem("nofap-relapses");
  ...
  // ✅ Har change pe localStorage mein save hoga
  const onToggle = (monthIdx, day) => {
    ...
    setRelapses(prev => {
      ...
      localStorage.setItem("nofap-relapses", JSON.stringify(next));
      return next;
    });
  };
